...
export function parseSmsText() {
    ...
    blocks.forEach((block, index) => {
        ...
        const findMatch = (str) => {
            if(!str) return null;
            const match = str.match(/[가-힣0-9]{2,}/);
        ...
        itemDiv.innerHTML = `
            ...
            <button type="button" onclick="window.applyParsedSms('${(fMatch||rawFrom).replace(/'/g, "\\'")}', '${(tMatch||rawTo).replace(/'/g, "\\'")}')" style="padding:8px; margin-top:8px; width:100%; background:#fab005; color:black; border-radius:4px; border:none; cursor:pointer; font-weight:bold;">입력창 채우기</button>
        `;
